...
var global_settings = woopo_apply_filters( 'woopo_global_settings', {} );
...
var store_obj = {
    state: {
        ...
        form_settings: woopo_reset_fields(woopo_form_settings),
        formdata:{
            form_type: 'contact',
            field_data: []
        }
    ...
    mutations: {
        ...
        add_row: function (state, {target_row}) {
            ...
            state.formdata.field_data.splice(target_row,0, woopo_reset_fields( state.row_template ) );
        },
        ...
        add_field: function (state,{field,target_row}) {
            state.formdata.field_data[target_row].row_formdata.push( woopo_reset_fields( field ) );
        },
        ...
        populate_form_type_data: function (state, {form_type, type, callback}) {
            /**
             * Ajax to fetch form data
             * @type {store_obj}
             * @private
             */
            var _this = this;
            jQuery.post(
                ajaxurl,
                {
                    action: 'woopo_populate_form_type_data',
                    form_type: form_type,
                    type: type
                },
                function (data) {
                    if( data.success === true ) {
                        state.formdata = woopo_reset_fields(data.data.form_type_data);
                    ...
                    state.formdata = woopo_reset_fields(form_data);*/
                }
            )
        ...
        update_form: function (state, {status, form_id, callback}) {
            ...
            jQuery.post(
                ajaxurl,
                {
                    action: 'woopo_update_form',
                    status: status,
                    title: state.form.post_title,
                    form_id: edit_form_id,
                    formdata: woopo_stringify(state.formdata),
                    form_settings: woopo_stringify(state.form_settings)
                },
                function (data) {
                    if(data.success) {
                        if( typeof callback == 'function' ) {
            ...
            jQuery.post(
                ...
                function (data) {
                    if(data.success) {
                        ...
                        state.formdata = data.data.formdata ? woopo_parse( data.data.formdata ) : [];

                        if( data.data.form_settings ) {
                            data.data.form_settings = woopo_parse( data.data.form_settings );
                        }

                        for( tabname in data.data.form_settings ) {
                            if( typeof state.form_settings[tabname] !== 'undefined' ) {
        ...
        reset_all: function (state, {}) {
            ...
            state.is_show_field_list = '';
            state.editing_mode = false;
            //state.form_settings = woopo_reset_fields(woopo_form_settings);
        },
        save_global_settings: function (state, {}) {
            jQuery.post(
                ajaxurl,
                {
                    action: 'woopo_save_global_settings',
                    global_settings: woopo_stringify(state.global_settings)
                },
                function (data) {
                    if(data.success) {
                        state.notice = {
        ...
        get_global_settings: function (state, {}) {
            jQuery.post(
                ajaxurl,
                {
                    action: 'woopo_get_global_settings'
                },
                function (data) {
                    if(data.success) {
                        state.global_settings = Object.assign({},state.global_settings,woopo_parse(data.data.global_settings));
                    }
                }
            )
        },
        ...
        clone_row: function (state, {}) {
            state.formdata.field_data.splice(state.focused_row,0,woopo_reset_fields(state.formdata.field_data[state.focused_row]));
        },
        clone_col: function (state, {}) {
            state.formdata.field_data[state.focused_row].row_formdata.splice( state.focused_col, 0, woopo_reset_fields( state.formdata.field_data[state.focused_row].row_formdata[state.focused_col] ) );
        },
        delete_fields: function (state, {}) {
            state.formdata.field_data = [];
        },
        copy_row: function (state, {row_number}) {
            state.copied_row = woopo_reset_fields( state.formdata.field_data[row_number] );
        },
        ...
        copy_col: function (state, {field_data}) {
            state.copied_col = woopo_reset_fields( field_data );
        },
        ...
        //populate taxonomies and terms
        populate_tax_terms: function (state) { return;
            jQuery.post(
                ajaxurl,
                {
                    action: 'woopo_get_tax_terms'
                },
                function (data) {
                }
            )
    ...
    actions: {
        ...
        populate_form_type_data:  function (context, {form_type, type, callback}) {
            context.commit('populate_form_type_data',{form_type:form_type, type: type, callback: callback});
        },
        ...
        populate_tax_terms: function (context) {
            context.commit('populate_tax_terms');
        }
    }
}
