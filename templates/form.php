<template id="woopo_new_form">
    <el-row>
        <el-col :sm="24">
            <div class="woopo-edit-mode">
                <div class="mb20">
                ...
                <el-row :gutter="10">
                    <el-col :sm="16">
                        <div class="oh mb20 oh">
                            <div>
                                <h5 class="mb10 mt0"><?php _e( 'Form Title', 'woopo' ); ?></h5>
                        ...
                        <el-tabs type="border-card">
                            <el-tab-pane label="<?php _e( 'Form Builder', 'woopo' ); ?>">
                                <div class="woopo_form_builder_panel">
                                    ...
                                    <woopo_new_form_root></woopo_new_form_root>
                                </div>
                            </el-tab-pane>
                            <el-tab-pane label="<?php _e( 'Settings', 'woopo' ); ?>">
			                    <?php $pages = get_pages();
			                    $neoform_pages = array();
                    ...
                    <el-col :sm="8">
                        <div class="mb20 action_panel mt30 oh">
                            <div class="alignleft">
                                <a href="javascript:" @click="update_form('draft')" class="button button-default"><?php _e( 'Save as Draft', 'woopo' ); ?></a>
                                <a v-if="!form_id || form.post_status !== 'publish'" href="javascript:" @click="update_form('publish')" class="button button-primary"><?php _e( 'Publish', 'woopo' ); ?></a>
                                <a v-if="form_id && form.post_status == 'publish'" href="javascript:" @click="update_form('publish')" class="button button-primary"><?php _e( 'Update', 'woopo' ); ?></a>
                                <a @click="delete_form(1)" href="javascript:" class="el-button el-button--mini el-button--danger"><i class="el-icon el-icon-delete"></i> <?php _e('Move to Trash','woopo'); ?></a>
                            </div>
                        </div>
                        <div>
                            <div class="woopo_tabs">
                                <div class="tabs oh">
                                    <a href=""><?php _e( 'Elements Panel', 'woopo' ); ?></a>
                                </div>
                                <div class="tab_cotnent p10">
                                    <div class="tab_pane">
                                        <div id="ui-woopo_panel_ground-1">
                                            <div class="woopo_element_tree">
	                                            <?php do_action('woopo_before_element_tree' ); ?>
                                                <template v-if="!is_multistep">
                                                    <ul>
                                                        <template v-for="(row_data,r) in formdata.field_data">
                                                            <li class="ui-woopo_row-1 woopo_tree_placeholder" v-if="row_data.type == 'row'"
                                                                :target_row="r" :data-row_number="r"
                                                            >
                                                                <span class="woopo_tree_placeholder_span pr woopo_row_mover">
                                                                    <?php _e( 'Placeholder', 'woopo' ); ?>
                                                                ...
                                                                <ul v-if="row_data.row_formdata.length && row_data == opened_tree_row">
                                                                    <template v-for="(field_data,c) in row_data.row_formdata">
                                                                        <li class="ui-woopo_col-1 woopo_tree_field"
                                                                            :target_col="c"
                                                                            :target_row="r"
                                                                        >
                                                                            <span class="woopo_tree_field_span pr woopo_col_mover">
                                                                                {{ field_data.s.label }}
                                                                            ...
                                                                            <div class="woopo_tree_field_settings" v-if="opened_tree_col === field_data">
    ...
    var woopo_new_form = {
        ...
        methods: {
            ...
            import_fields: function ( from, to ) {
                ...
                var self = this;
                setTimeout(function () {
                    woopo_reset_sortable(self,self.is_multistep);
                },1);
            },
            ...
            fetchData: function () {
                ...
                if ( typeof  this.$route.params.form_type !== 'undefined' ) {
                    var _this = this;
                    /*if( typeof woopo_form_type_data[this.$route.params.form_type] !== 'undefined') {*/
                    this.$store.dispatch('populate_form_type_data',
                        {
                            form_type: _this.$route.params.form_type,
                            type: typeof _this.$route.params.form_type !== 'undefined' ? _this.$route.params.type : '',
                            callback: function () {
                                _this.update_form('publish');
                            }
                        }
                    );
                }
                ...
                //if edit form
                    ...
                    this.$store.dispatch('get_form',{id:this.$route.params.form_id, callback: function () {
                        return;
                        /*woopo_reset_sortable_row(self, self.is_multistep);
                        setTimeout(function () {
                            woopo_reset_sortable_field(self, self.is_multistep);
                        },1);*/
                    }});
                }
                 ...
                 */
                this.$store.dispatch( 'populate_tax_terms' );
            }
        },
        computed: {
            field_attr: function () {
...
<template id="woopo_row">
    <div class="ui-woopo_row" @mouseover="make_focused_row(row_number)" @mouseout="defocus_row(row_number)" :target_row="row_number" :data-row_number="row_number" :class="{focused_row:is_focused_row}">
        <el-row style="text-align: center;">
            <woopo_row_panel :target_row="row_number" v-if="is_focused_row && is_editing"></woopo_row_panel>
            <template v-if="form_data.length">
                <template v-for="(field_data,k) in form_data">
                    <woopo_input_template v-if="field_data.type == 'input' || field_data.inputType == 'text'"
                                             :field_data="field_data" :target_row="row_number" :target_col="k"></woopo_input_template>
                </template>
            </template>
            <template v-else>
...
<template id="woopo_new_form_root">
    <!--if -->
    <div id="ui-woopo_builder_ground">

        <div :class="'woopo_layout_' + form_settings.appearance_settings.s.layout_type">
            <el-form ref="form" label-width="120px" method="post">
                <div class="woopo_db woopo_center pr">
                    <template v-if="is_editing">
                        <a href="javascript:" @click="add_row()" class="woopo_btn-add_row" title="<?php _e( 'Add Placeholder', 'woopo' ); ?>">
                            <i class="dashicons dashicons-plus-alt"></i> <?php _e( 'Add Placeholder', 'woopo' ); ?>
                        </a>
                    </template>
                    <a href="javascript:" @click="set_is_editing()" v-if="!is_editing" class="woopo_btn-add_row" title="<?php _e( 'Edit', 'woopo' ); ?>"><i class="dashicons dashicons-edit"></i> <?php _e( 'Edit', 'woopo' ); ?></a>
                    <a href="javascript:" @click="unset_is_editing()" v-if="is_editing" class="woopo_btn-add_row" title="<?php _e( 'Preview', 'woopo' ); ?>"><i class="dashicons dashicons-visibility"></i> <?php _e( 'Preview', 'woopo' ); ?></a>
                    <?php do_action('woopo_formbuilder_header'); ?>
                </div>
                <?php do_action('woopo_before_formbuilder' ); ?>
                <template v-if="!is_multistep">
                    <template v-for="(row_data,k) in field_data" v-if="render_field_list">
                        <woopo_row v-if="row_data.type == 'row'" :row_number="k" :form_data="row_data.row_formdata"></woopo_row>
                    </template>
                </template>
            </el-form>
        </div>
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('woopo_new_form_root',{
            template: '#woopo_new_form_root',
            data: function () {
                return {
...
<template id="woopo_input_template">
    ...
    >
        <div @mouseover="make_focused_col(target_col)" @mouseout="defocus_col(target_col)"
            ...
            <woopo_input_panel :field_data="field_data" v-if="is_focused_col() && is_editing"></woopo_input_panel>
        </div>
    </el-col>
</template>
<script>
...
<template id="woopo_input_panel">
    <div class="woopo_input_panel">
        <a href="javascript:" @click="show_grids = true;set_edit_mode();" title="<?php _e( 'Resize', 'woopo' ); ?>">
            <i class="dashicons dashicons-leftright"></i>
        </a>
        <a href="javascript:" @click="open_settings = !open_settings;toggle_edit_mode();" title="<?php _e( 'Settings', 'woopo' ); ?>">
        ...
        <a href="javascript:" @click="remove_field()" title="<?php _e( 'Remove', 'woopo' ); ?>">
            <i class="dashicons dashicons-trash"></i>
        </a>
        <a href="javascript:" class="woopo_col_mover" title="<?php _e( 'Move Field'); ?>"><i class="dashicons dashicons-move"></i></a>
        <div v-if="open_settings" class="woopo-field-settings-panel">
            <div class="woopo_center mb5">
                <a href="javascript:" class="woopo_btn-flat" @click="open_settings = false;unset_edit_mode();"><?php _e( 'Save', 'woopo' ); ?></a>
                <a href="javascript:" class="woopo_btn-flat" @click="cancel_editing();open_settings = false;unset_edit_mode();"><?php _e( 'Cancel', 'woopo' ); ?></a>
            </div>
            <el-tabs type="border-card">
        ...
        <div class="woopo_grid_panel" v-if="show_grids">
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('woopo_input_panel',{
            template: '#woopo_input_panel',
            props: ['field_data'],
            data: function () {
                return {
                    show_grids: false,
            ...
            methods: {
                ...
                toggle_edit_mode: function () {
                    if( this.open_settings ) {
                        ...
                        this.temp_col_formdata = woopo_reset_fields(this.col_formdata.s);
                    } else {
                ...
                cancel_editing: function () {
                    if( Object.keys(this.temp_col_formdata).length ) {
                        this.col_formdata.s = woopo_reset_fields( this.temp_col_formdata );
                    }
                },
...
<template id="woopo_row_panel">
    <div class="row-panel pr">
        <a href="javascript:" @click="show_field_list();set_edit_mode();" v-if="!is_show_field_list"><i class="dashicons dashicons-plus-alt2"></i> <?php _e( 'Add Field', 'woopo' ); ?></a>
        <a href="javascript:" @click="hide_field_list();unset_edit_mode();" v-if="is_show_field_list"><i class="dashicons dashicons-no-alt"></i> <?php _e( 'Close Panel', 'woopo' ); ?></a>
        <a href="javascript:" @click="add_row(target_row)" title="<?php _e( 'Add Placeholder', 'woopo' ); ?>"><i class="dashicons dashicons-plus-alt"></i> <?php _e( 'Add Placeholder', 'woopo' ); ?></a>
        <a href="javascript:" @click="clone_row(target_row)" title="<?php _e( 'Clone Placeholder', 'woopo' ); ?>"><i class="dashicons dashicons-welcome-add-page"></i></a>
        <a href="javascript:" @click="copy_row()" title="<?php _e( 'Copy Placeholder', 'woopo' ); ?>">
            <i class="dashicons dashicons-admin-page"></i>
        </a>
        <a href="javascript:" v-if="copied_row" @click="paste_row()"><i class="dashicons dashicons-category"></i> <?php _e( 'Paste Placeholder', 'woopo' ); ?></a>
        <a href="javascript:" v-if="copied_col" @click="paste_col()"><i class="dashicons dashicons-feedback"></i><?php _e( 'Paste Field', 'woopo' ); ?></a>
        <a href="javascript:" @click="remove_row(target_row)" title="<?php _e( 'Remove Placeholder', 'woopo' ); ?>"><i class="dashicons dashicons-trash"></i></a>
        <a href="javascript:" class="woopo_row_mover" title="<?php _e( 'Move Placeholder'); ?>" title="<?php _e( 'Sort', 'woopo' ); ?>"><i class="dashicons dashicons-move"></i></a>
        <woopo_field_list :target_row="target_row"></woopo_field_list>
    </div>
</template>
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('woopo_row_panel',{
            template:'#woopo_row_panel',
            props:['target_row'],
            data: function () {
                return {
                    open_menu: false,
...
<script>
    ;document.addEventListener("DOMContentLoaded", function(event) {
        Vue.component('woopo_field_list',{
            template: '#woopo_field_list',
            props: ['target_row'],
            data: function () {
                return {
